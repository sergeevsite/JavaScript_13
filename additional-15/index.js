...
class AppData {
  ...
  addExpIncBlock(event) {
    let items = event.target.previousElementSibling;
    let cloneItem = items.cloneNode(true);
    items.parentNode.insertBefore(cloneItem, event.target);
    items = document.querySelectorAll(`.${items.className}`);
    if(items.length === 3) {
